  ...
  switch (op.toLowerCase()) {
    ...
    case "add": {
      ...
      if (checkOverflow(result)) {
        ...
        registers[rd] = result;
        console.log(`ADD: ${rs}(${registers[rs]}) + ${rt}(${registers[rt]}) = ${result}, guardado en ${rd}`);
      }
      break;
    }
    case "addu": {
      ...
      if (checkOverflow(result)) {
        ...
        registers[rd] = result;
        console.log(`ADDU: ${rs}(${registers[rs]}) + ${rt}(${registers[rt]}) = ${result}, guardado en ${rd}`);
      }
      break;
    }
    case "sub": {
    ...
    case "addi": {
      ...
      if (checkOverflow(result)) {
        ...
        registers[rt] = result;
        console.log(`ADDI: ${rs}(${registers[rs]}) + ${imm} = ${result}, guardado en ${rt}`);
      }
      break;
    }
    case "addiu": {
      ...
      if (checkOverflow(result)) {
        ...
        registers[rt] = result;
        console.log(`ADDIU: ${rs}(${registers[rs]}) + ${imm} = ${result}, guardado en ${rt}`);
      }
      break;
    }
    ...
    case "lw": {
      ...
      if (match) {
        ...
        if (address < 0 || address >= Object.keys(memory).length) {
          setErrorMessage(`Error: Acceso a dirección de memoria inválida: ${address}`);
          console.error(`Error: Acceso a dirección de memoria inválida: ${address}`);
          break;
        }
        
        registers[rt] = memory[address] || 0;
        console.log(`LW: Cargando valor ${memory[address]} desde dirección ${address} a registro ${rt}`);
      } else {
    ...
    case "sw": {
      ...
      if (match) {
        ...
        console.log(`SW: Guardando valor ${registers[rt]} en dirección ${address} desde registro ${rt}`);
      } else {
    ...
    case "lb": {
      ...
      if (match) {
        ...
        if (byte & 0x80) {  // Si el bit más significativo está activo, extender signo
          byte = byte | 0xFFFFFF00;
        }
        
        registers[rt] = byte;
        console.log(`LB: Cargando byte ${byte} desde dirección ${address} a registro ${rt}`);
      } else {
    ...
    case "lbu": {
      ...
      if (match) {
        ...
        registers[rt] = byte;
        console.log(`LBU: Cargando byte sin signo ${byte} desde dirección ${address} a registro ${rt}`);
      } else {
    ...
    case "sb": {
      ...
      if (match) {
        ...
        console.log(`SB: Guardando byte ${byte} en dirección ${address} desde registro ${rt}`);
      } else {
